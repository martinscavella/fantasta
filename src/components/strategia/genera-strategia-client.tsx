...
export function GeneraStrategiaClient({ setup, giocatori }: { setup: SetupDoc; giocatori: Player[] }) {
  ...
  return (
    <div className="mx-auto flex max-w-2xl flex-col gap-6 p-6 md:p-8">
      ...
      <PageHeader
        title="Genera strategia"
        description="Compila i parametri, copia il prompt e incollalo in una chat Claude sul tuo abbonamento. Claude farà ricerca sul web e risponderà con un blocco JSON: incollalo qui sotto per validarlo e applicarlo alla strategia."
        actions={
          <Button variant="outline" size="sm" nativeButton={false} render={<Link href={`/strategia/${setup.id}`} />}>
            Torna alla strategia
          </Button>
        }
      />

      <SectionCard title="Parametri" icon={Sparkles}>
        <div className="flex flex-col gap-3">
          ...
          <Button type="button" className="w-fit" onClick={genera}>
            Genera prompt
          </Button>
        </div>
      </SectionCard>
      ...
      <SectionCard title="Incolla qui la risposta">
        <div className="flex flex-col gap-2">
          ...
          <Button type="button" className="w-fit" onClick={() => void valida()} disabled={pending || !risposta.trim()}>
